export const brand = {
  name: "Cohortify",
  tagline: "Where ambition meets alliance.",
  email: "hello@cohortify.placeholder",
  location: "Kathmandu, Nepal • Online sessions",
};

export const session = {
  label: "Upcoming Session 04",
  title: "Padhai se Aage — Streams, Skills aur Smart Choices",
  date: "Saturday, 26 September 2026",
  time: "7:00 PM – 8:30 PM (NPT)",
  mode: "Online • Zoom link WhatsApp group mein",
  age: "Students 13–16 & Parents welcome",
  fee: "NPR 250",
  enrolled: 5,
  seats: 9,
  remaining: 4,
  description:
    "Ek session mein poora clarity pack. Mentor aapko batayega ki Class 9–10 mein kaunse decisions actually matter karte hain — stream choice, study routine, distraction control aur confidence. Saath mein alag-alag schools ke students se seedha interaction, aur host ke saath halke-fulke jokes bhi.",
  agenda: [
    "Stream aur subject choice: kaise sochna chahiye",
    "Study routine jo sach mein follow ho paaye",
    "Distraction, phone aur focus ka simple system",
    "Open networking round — naye dost, naye schools",
  ],
};

export const mentor = {
  name: "Aarav Mehta",
  grade: "Class 12 • Science",
};

export const host = {
  name: "Shishamt Sah",
  grade: "Organizer • Cohortify",
  role: "Host",
};

export const navLinks = [
  ["Upcoming Session", "upcoming-session"],
  ["Kya Milega", "what-you-get"],
  ["Mentor", "mentor"],
  ["Gallery", "gallery"],
  ["Reviews", "testimonials"],
  ["Pricing", "pricing"],
];

export const benefits = [
  [
    "ri-user-star-line",
    "Topper Mentorship",
    "Har session mein ek naya high-achieving mentor jo khud Class 11/12 mein hai — jo abhi-abhi wahi phase cross kiya hai jo aap abhi jeete ho.",
  ],
  [
    "ri-compass-3-line",
    "Future + Personal Guidance",
    "Stream, subject aur career ki direction, saath mein confidence, discipline aur distraction handling par seedhi baat.",
  ],
  [
    "ri-links-line",
    "Cross-School Networking",
    "Different schools ke students ek hi table par. Naye perspectives, naye ideas aur ek network jo school ke bahar bhi kaam aata hai.",
  ],
  [
    "ri-hand-heart-line",
    "Sachhi Dosti",
    "Session ke baad bhi baat jaari — ek cohort jo aapko akela feel nahi hone deta.",
  ],
  [
    "ri-emotion-laugh-line",
    "Fun, Jokes & Interaction",
    "Lecture nahi, conversation. Memes, light jokes aur live interaction jisse 90 minutes nikal jaate hain pata bhi nahi chalta.",
  ],
  [
    "ri-gift-2-line",
    "Study Material & Bumper Prizes",
    "Session ke saath useful study materials, aur selected sessions mein bumper prizes ka chance.",
  ],
  [
    "ri-whatsapp-line",
    "Temporary Session WhatsApp Group",
    "Har session ka apna WhatsApp group — doubt, discussion aur announcements ke liye.",
  ],
  [
    "ri-parent-line",
    "Parents Ke Liye Clarity",
    "Parents bhi join kar sakte hain aur dekhte hain ki bachcha kya seekh raha hai.",
  ],
];

export const testimonials = [
  [
    "Saanvi, Class 10",
    "Pehli baar stream choice itni simple aur non-scary lagi. Plus, I actually made friends outside my school.",
  ],
  [
    "Aarav ke parent",
    "Session ke baad jo summary mili usse humein bhi samajh aaya ki Aarav kis direction mein soch raha hai.",
  ],
  [
    "Nischal, Class 9",
    "Lecture jaisa bilkul nahi tha. Mentor bhaiya ne phone distraction ka proper system diya.",
  ],
];

export const faqs = [
  [
    "Kya parents join kar sakte hain?",
    "Bilkul. Parents welcome hain aur session ke baad unhe short summary bhi milti hai.",
  ],
  [
    "Session online kaise hoga?",
    "Zoom link temporary WhatsApp group mein share kiya jayega.",
  ],
  [
    "Kya main single session ke bajay pass le sakta hoon?",
    "Haan, 4-month pass NPR 800 mein available hai.",
  ],
  [
    "Age group kya hai?",
    "Primary focus 13–16 saal ke students hain, parents ke saath.",
  ],
];

export const images = {
  hero:
    "https://readdy.ai/api/search-image?query=Abstract%20artistic%20background%20with%20warm%20terracotta%20orange%20and%20deep%20amber%20gradients%2C%20soft%20flowing%20geometric%20shapes%20and%20layered%20organic%20curves%2C%20subtle%20silhouettes%20of%20young%20students%20in%20conversation%2C%20cinematic%20dark%20vignette%20at%20the%20edges%20for%20strong%20text%20contrast%2C%20premium%20editorial%20poster%20look&width=1920&height=1080&seq=cohortify-hero-main&orientation=landscape",
  cta:
    "https://readdy.ai/api/search-image?query=Warm%20abstract%20artistic%20backdrop%20with%20deep%20terracotta%20and%20golden%20amber%20gradient%20waves%2C%20soft%20organic%20layered%20curves%20and%20subtle%20grain%20texture%2C%20slight%20silhouette%20of%20teenage%20students%20standing%20together%2C%20dark%20rich%20vignette%20for%20text%20contrast%2C%20premium%20modern%20poster%20aesthetic&width=1920&height=900&seq=cohortify-cta-band&orientation=landscape",
  mentor:
    "https://readdy.ai/api/search-image?query=Confident%20teenage%20Nepali%20male%20student%20mentor%20in%20smart%20casual%20shirt%20smiling%20warmly%2C%20studio%20portrait%20with%20soft%20warm%20cream%20background%2C%20natural%20window%20light%2C%20editorial%20photography%2C%20premium%20youthful%20look%2C%20sharp%20detail%2C%20friendly%20approachable%20energy&width=900&height=1100&seq=cohortify-mentor-portrait&orientation=portrait",
  host:
    "https://readdy.ai/api/search-image?query=Young%20South%20Asian%20male%20organizer%20host%20in%20smart%20casual%20outfit%20with%20relaxed%20confident%20smile%2C%20clean%20soft%20cream%20studio%20backdrop%2C%20warm%20even%20lighting%2C%20professional%20editorial%20portrait%2C%20approachable%20welcoming%20expression%2C%20high%20detail&width=900&height=1100&seq=cohortify-host-portrait&orientation=portrait",
  gallery: [
    "https://readdy.ai/api/search-image?query=Group%20of%20teenage%20Nepali%20students%20laughing%20together%20in%20a%20bright%20modern%20meeting%20room%20with%20warm%20cream%20walls%20and%20wooden%20furniture%2C%20candid%20documentary%20photography%2C%20natural%20window%20light%2C%20warm%20joyful%20community%20energy&width=1000&height=750&seq=cohortify-gallery-01&orientation=landscape",
    "https://readdy.ai/api/search-image?query=Teenage%20student%20mentor%20presenting%20in%20front%20of%20a%20small%20group%20of%20younger%20students%20in%20a%20warm%20cozy%20workshop%20space%2C%20soft%20ambient%20lighting%2C%20candid%20documentary%20photography%2C%20orange%20and%20cream%20color%20tones&width=1000&height=750&seq=cohortify-gallery-02&orientation=landscape",
    "https://readdy.ai/api/search-image?query=South%20Asian%20parents%20sitting%20and%20listening%20attentively%20in%20a%20bright%20informal%20session%20circle%20with%20soft%20warm%20lighting%2C%20candid%20authentic%20documentary%20photography%2C%20cream%20and%20amber%20tones&width=1000&height=750&seq=cohortify-gallery-03&orientation=landscape",
    "https://readdy.ai/api/search-image?query=Diverse%20group%20of%20Nepali%20teenagers%20chatting%20and%20laughing%20casually%20during%20a%20break%20in%20a%20sunlit%20warm%20hall%2C%20candid%20lifestyle%20documentary%20photography%2C%20joyful%20friendly%20atmosphere%2C%20soft%20golden%20light&width=1000&height=750&seq=cohortify-gallery-04&orientation=landscape",
    "https://readdy.ai/api/search-image?query=Excited%20teenage%20students%20raising%20hands%20during%20a%20fun%20interactive%20quiz%20in%20a%20warm%20modern%20room%2C%20energetic%20candid%20event%20photography%2C%20warm%20amber%20and%20cream%20palette%2C%20natural%20light&width=1000&height=750&seq=cohortify-gallery-05&orientation=landscape",
    "https://readdy.ai/api/search-image?query=Friendly%20group%20photo%20of%20teenage%20students%20and%20a%20young%20mentor%20posing%20together%20in%20a%20warm%20bright%20indoor%20space%2C%20happy%20community%20celebration%20moment%2C%20candid%20documentary%20style%2C%20soft%20cream%20tones&width=1000&height=750&seq=cohortify-gallery-06&orientation=landscape",
  ],
  avatars: [
    "https://readdy.ai/api/search-image?query=Friendly%20teenage%20Nepali%20school%20student%20portrait%20headshot%20with%20soft%20smile%2C%20plain%20warm%20cream%20background%2C%20even%20natural%20lighting%2C%20clean%20modern%20yearbook%20style%20photograph%2C%20sharp%20focus%20on%20face&width=240&height=240&seq=cohortify-review-avatar-1&orientation=squarish",
    "https://readdy.ai/api/search-image?query=Cheerful%20South%20Asian%20teenage%20girl%20student%20portrait%20headshot%20with%20bright%20friendly%20smile%2C%20soft%20cream%20plain%20background%2C%20gentle%20diffused%20lighting%2C%20modern%20clean%20portrait%20photography%2C%20crisp%20detail&width=240&height=240&seq=cohortify-review-avatar-2&orientation=squarish",
    "https://readdy.ai/api/search-image?query=Middle%20aged%20South%20Asian%20father%20portrait%20headshot%20with%20calm%20warm%20smile%2C%20simple%20neutral%20cream%20background%2C%20soft%20studio%20lighting%2C%20professional%20friendly%20photograph%2C%20clear%20sharp%20detail&width=240&height=240&seq=cohortify-review-avatar-3&orientation=squarish",
  ],
};

images.fallbackHero =
  "https://images.unsplash.com/photo-1523240795612-9a054b0db644?auto=format&fit=crop&w=1920&q=85";
images.fallbackCta =
  "https://images.unsplash.com/photo-1529390079861-591de354faf5?auto=format&fit=crop&w=1920&q=85";
images.fallbackMentor =
  "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=900&q=85";
images.fallbackHost =
  "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?auto=format&fit=crop&w=900&q=85";
images.fallbackGallery = [
  "https://images.unsplash.com/photo-1529156069898-49953e39b3ac?auto=format&fit=crop&w=1000&q=85",
  "https://images.unsplash.com/photo-1524178232363-1fb2b075b655?auto=format&fit=crop&w=1000&q=85",
  "https://images.unsplash.com/photo-1543269865-cbf427effbad?auto=format&fit=crop&w=1000&q=85",
  "https://images.unsplash.com/photo-1529156069898-49953e39b3ac?auto=format&fit=crop&w=1000&q=85",
  "https://images.unsplash.com/photo-1531482615713-2afd69097998?auto=format&fit=crop&w=1000&q=85",
  "https://images.unsplash.com/photo-1511632765486-a01980e01a18?auto=format&fit=crop&w=1000&q=85",
];
images.fallbackAvatars = [
  "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=240&q=85",
  "https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=240&q=85",
  "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=240&q=85",
];